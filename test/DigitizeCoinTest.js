...
const ApproveAndCallFallbackMock = artifacts.require("./interfaces/ApproveAndCallFallbackMock.sol");
...
contract('DigitizeCoin', (accounts) => {
    ...
    it("Fresh token has correct initial values", async () => {
        assert("Digitize Coin" == await token.name.call());
        assert("DTZ" == await token.symbol.call());
    ...
    it("Only owner can withdraw arbitrary tokens sent to this token", async () => {
        let otherTokentoken = await DigitizeCoin.new();
    ...
    it("ApproveAndCall works correctly", async () => {
        let mock = await ApproveAndCallFallbackMock.new();
        let failingMock = await DigitizeCoin.new();
        ...
        try {
            ...
            expectRevert(e, "digitize token doesn't implement token fallback");
        }
    });
